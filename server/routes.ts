...
import {
  ...
  views,
  stageAutomations,
  insertCustomPropertySchema,
  insertCustomPropertyValueSchema,
  insertViewSchema,
  insertStageAutomationSchema,
} from "@shared/schema";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/tasks", requireAuth, async (req: any, res) => {
    try {
      ...
      // Fetch custom properties for the tasks
      let enrichedTasks = tasks;
      if (tasks.length > 0) {
        const taskIds = tasks.map(t => t.id);
        const properties = await db.select().from(customPropertyValues).where(inArray(customPropertyValues.entityId, taskIds));
        
        enrichedTasks = tasks.map(task => {
          const taskProps = properties.filter(p => p.entityId === task.id);
          const propertyMap = taskProps.reduce((acc, curr) => {
            acc[curr.propertyId] = curr.value;
            return acc;
          }, {} as Record<string, any>);
          return { ...task, properties: propertyMap };
        });
      }
  ...
  app.put("/api/tasks/:id", requireAuth, async (req: any, res) => {
    try {
      ...
      if (data.stageId && data.stageId !== oldTask.stageId) {
        ...
        const automations = await db.select().from(stageAutomations).where(and(
          eq(stageAutomations.stageId, data.stageId),
          eq(stageAutomations.trigger, "ON_ENTER")
        ));

        for (const auto of automations) {
          console.log(`[Automation] Executing ${auto.actionType} for Task '${task.title}'`);
          // e.g., if (auto.actionType === "SET_CLIENT_VISIBILITY") { ... }
        }
      }
